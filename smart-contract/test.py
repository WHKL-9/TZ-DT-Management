...
class Marketplace(sp.Contract):
    ...
    def collect(self, params):
        ...
        # sending rewards
        sp.send(self.data.data[params.token_id].author,
                sp.split_tokens(sp.amount, 97, 100))
...
def test():
    ...
    CONTI = sp.test_account("user1")
    BMW = sp.test_account("user2")
    ...
    marketplace = Marketplace(token_contract.address, sp.utils.metadata_of_url(
        "ipfs://QmW8jPMdBmFvsSEoLWPPhaozN6jGQFxxkwuMLtVFqEy6Fb"), ADMIN.address)
    ...
    scenario += marketplace.mint(sp.record(amount=100000000, metadata=sp.pack(
        "ipfs://bafyreibwl5hhjgrat5l7cmjlv6ppwghm6ijygpz2xor2r6incfcxnl7y3e/metadata.json"))).run(sender=ADMIN)
    scenario += marketplace.mint(sp.record(amount=5600000,
                                 metadata=sp.pack("123423"))).run(sender=BMW)
    scenario.h1("Collect")
    scenario += marketplace.collect(sp.record(token_id=1)
                                    ).run(sender=CONTI, amount=sp.mutez(5600000))
